...
# Load the image and converti to grayscale
...
#find countours in the edge map
...
for((xA,yA),(xB,yB),color) in zip(refCoords,objCoords,colors):
    ...
    Dm = dist.jensenshannon((xA, yA), (xB, yB)) / refObj[2]
    (mX,mY) = midpoint((xA,yA),(xB,yB))
    cv2.putText(orig,"{:.1f}in".format(D),(int(mX),int(mY - 10)),cv2.FONT_HERSHEY_SIMPLEX,0.55,color,2)
    cv2.putText(orig, "{:.1f}cm".format(D*2.54), (int(mX + 100), int(mY - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
